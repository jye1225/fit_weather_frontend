...
const CodiWrite = () => {
  ...
  useEffect(() => {
    ...
    if (storedRegionSecondName) {
      if (storedRegionSecondName.split(" ").length === 2) {
        // '부천시 원미구' 처럼 두 단어일때
  ...
  const handleBtnClick = (option) => {
    if (activeOptions.includes(option)) {
      const updateActiveOptions = activeOptions.filter(
        (item) => item !== option
      );
  ...
  useEffect(() => {
    if (file) {
      ...
      setFileMss("");
      document.querySelector(`.${style.imgCon}`).style.border =
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/codiWrite`, {
        method: "POST",
        body: data,
        credentials: "include", // 쿠키 주고받기 위한 설정
      });
  ...
  return (
    <main className={`mw ${style.codiWrite}`}>
      ...
      <section className={style.codiWriteFrame}>
        <h3 className="fontHead3">
          {codiDate.split("-")[0]}년 {codiDate.split("-")[1]}월{" "}
          {codiDate.split("-")[2]}일
        </h3>
        <div className={style.weatherInfo}>
          <span className={`fontTitleXS ${style.miniTag}`}>{tagAddress}</span>
          <span className={`fontTitleS ${style.temp}`}>
            {maxTemp}°C/{minTemp}°C
          </span>
            ...
            {feltOptions.map((option, index) => (
              <button
                ...
                className={`${style.BtnToggle} fontBodyM ${
                  activeOptions.includes(option) ? style.active : ""
                }`}
        ...
        <div className={style.Qmemo}>
          <label htmlFor="memo" className="fontHead3">
            오늘의 코디 메모!
          </label>
          <textarea
            className="fontBodyM"
      ...
      <div className={style.bottomBtns}>
        ...
        <button
          className={`fontTitleM ${style.btnMethod}`}
