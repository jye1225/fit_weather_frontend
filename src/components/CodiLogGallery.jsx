        ...
        if (codiLogList.length === index + 1) {
          return (
            <li
              ...
              onClick={() => setModalActive(codiLog._id)}
            >
              <div className={style.dateBox}>
                <span className={`fontTitleM ${style.day}`}>
                  {codiLog.codiDate.slice(8, 9) === '0'
                    ? codiLog.codiDate.slice(9, 10)
                    : codiLog.codiDate.slice(8, 10)}
                </span>
                <span className={`fontBodyS ${style.month}`}>
                  {codiLog.codiDate.slice(5, 6) === '0'
                    ? codiLog.codiDate.slice(6, 7)
                    : codiLog.codiDate.slice(5, 7)}
                  월
                </span>
              </div>
